...
float area_holding, area_shortage, holding_cost, incremental_cost, maxlag, mean_interdemand, minlag, prob_distrib_demand[26], setup_cost, shortage_cost,
time, time_last_event, time_next_event[5], total_ordering_cost;
...
int main() /* Main function. */
{
	...
	fscanf(infile, "%d %d %d %d %f %f %f %f %f %f %f", &initial_inv_level, &num_months, &num_policies, &num_values_demand, &mean_interdemand, &setup_cost,
		&incremental_cost, &holding_cost, &shortage_cost, &minlag, &maxlag);
	...
	for (i = 1; i <= num_policies; ++i)
	{
		...
		do {
			...
			/* If the event just executed was not the end-simulation event (type 3), continue simulating. Otherwise, end the simulation for the current (s,S)
...
void initialize(void) /* Initialization function. */
{
	...
	time_next_event[1] = time + expon(mean_interdemand);;
		
	time_next_event[2] = 1.0e+30;
	time_next_event[3] = num_months;
	time_next_event[4] = 0.07;
}
...
void evaluate(void) /* Inventory-evaluation event function. */
{
	...
	if (inv_level < smalls)
	{
		/* The inventory level is less than smalls,· so pi ace an order for the appropriate amount. */
...
void report(void) /* Report generator function. */
{
	...
	fprintf(outfile, "\n\n (%3d, %3d) %15.2f %15.2f %15.2f %15.2f", smalls, bigs, avg_ordering_cost + avg_holding_cost + avg_shortage_cost, avg_ordering_cost,
...
void update_time_avg_stats(void) /* Update area accumulators for time-average statistics. */
{
	...
	/* Determine the status of the inventory level during previous interval. If the inventory level during the previous interval was negative,
	   update area_shortage. If, it, was positive, update area_holding. If it was zero, no update is needed. */
...
float expon(float mean)	/* Exponential Variate Generator Fucntion */
{
	float u;
	...
	/* Return a Random Exponential Variate with Mean "mean" */
	float newvar= mean * log(u);
	return newvar;
}
...
float uniform(float a, float b) /* Uniform variate generation function. */
{
	...
	/* Generate a U(O,1) random variate. */
