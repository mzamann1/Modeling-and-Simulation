...
int next_event_type, num_custs_delayed, num_delays_required, num_events, num_in_q, server_status, stopping_cond,balk_cust=0;
...
int main(int argc, char const *argv[]) 
{
	...
	fscanf(infile, "%f %f %d", &mean_interarrival, &mean_service, &stopping_cond); //&num_delays_required
	printf("%d\n", stopping_cond);
...
void report(void)
{
	...
	fprintf(outfile, "Number of Customer Delayed  :  %d", num_custs_delayed);
	fprintf(outfile, "Balked Customers: %d\t", balk_cust);
